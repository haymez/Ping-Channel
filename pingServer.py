...
if(len(sys.argv) < 2):
	...
	while(1):
		...
		sniff(iface="wlan", filter="icmp and (src " + str(args.s) + ")", prn=listener, count=1)
		
		#sniff for message and timeout after 8 seconds
		sniff(iface="wlan", filter="icmp and (src " + str(args.s) + ")", prn=listener, stop_filter=stopListening)
